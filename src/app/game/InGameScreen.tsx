...
export function InGameScreen({ onNextRound, onQuitToMenu }: InGameScreenProps) {
  ...
  const [mutationPopup, setMutationPopup] = useState<{
    name: string;
    counter: string | null;
  } | null>(null);
  ...
  useEffect(() => {
    const t1 = setTimeout(() => {
      ...
      if (state.roundNumber === 1) {
        ...
        const newMutation =
          state.activeMutations[state.activeMutations.length - 1];
        const displayName = newMutation
          .replace("_Mutation", "")
          .replace(/_/g, " ")
          .toUpperCase();
        ...
        const counterName = counter
          ? (DEFENSE_DISPLAY_NAMES[counter] ?? counter)
          : null;
  ...
  const actions = [
    {
      ...
      description: "Deploy immune cells to patrol the zone",
      color: "#1D9E75",
    },
    {
      id: "Antibodies",
      name: "Antibody Production",
      description: "Target specific viral strains",
      color: "#1D9E75",
    },
    {
      id: "Inflammation",
      name: "Inflammation",
      description: "Quarantine the zone (causes minor host damage)",
      color: "#EF9F27",
    },
    {
      id: "FeverResponse",
      name: "Fever Response",
      description: "Global viral slowdown for two rounds",
      color: "#EF9F27",
    },
    {
      id: "MemoryCells",
      name: "Memory Cells",
      description: "Counter previously seen mutations rapidly",
      color: "#1D9E75",
    },
    {
      id: "CytokineBurst",
      name: "Cytokine Burst",
      description: "Massive damage (+10% Severity penalty)",
      color: "#E24B4A",
    },
  ];
  ...
  const getMutationCounterLabel = (actionId: string): string | null => {
    for (const mutation of state.activeMutations) {
      if (DecisionTree.MutationCounters[mutation] === actionId) {
        const mutName =
          MUTATION_READABLE[mutation] ?? mutation.replace("_Mutation", "");
  ...
  const mappedOrgansForBodyMap = Object.values(state.organGraph.zones).map(
    (organ: any) => {
      let infectionPercentage = 0;
      if (organ.isInfected) {
        const cureProgress =
          (organ.reclamationProgress / RECLAMATION_THRESHOLD) * 100;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#1a3a2a]">
        <div className="flex items-center gap-6">
          <div>
            ...
            <div className="text-[#e8f5f0] text-2xl font-bold">
              {state.roundNumber}
            </div>
          </div>
          <div>
            ...
            <div className="text-[#EF9F27] text-sm font-bold uppercase">
              {state.difficulty}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex gap-4">
        ...
        <div className="w-64 flex flex-col space-y-4">
            ...
            <div className="relative">
              <div className="w-full h-4 bg-[#0a1f12] border border-[#1a3a2a] rounded-sm overflow-hidden">
                <motion.div
                  ...
                  style={{
                    background:
                      state.severityIndex > 75
                        ? "#E24B4A"
                        : state.severityIndex > 40
                          ? "#EF9F27"
                          : "#1D9E75",
                  }}
                />
          ...
          <div>
            ...
            <div className="text-[#1D9E75] text-3xl font-bold tracking-wider">
              {state.playerEp}
            </div>
          ...
          <div>
            ...
            <div className="text-[#EF9F27] text-3xl font-bold tracking-wider">
              {state.infectionRate}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="border-2 border-[#EF9F27] bg-[#1a0d00] p-2 rounded-sm min-h-[40px]">
              {acknowledgedMutations.length === 0 ? (
                <div className="text-[#EF9F27] text-xs tracking-[1px] text-center opacity-50">
                  NONE
                </div>
              ) : (
                acknowledgedMutations.map((m) => (
                  <div
                    key={m}
                    className="text-[#EF9F27] text-xs tracking-[1px] text-center font-bold mb-1"
                  >
          ...
          {dominantWarning && (
            ...
            >
              <div className="text-[#EF9F27] text-[10px] tracking-[2px] font-bold mb-1">
                ⚠ ADAPTATION RISK
              </div>
          ...
          <div className="flex-1">
            <div className="text-[#5DCAA5] text-xs tracking-[2px] mb-3 mt-4">
              ORGAN STATUS
            </div>
                ...
                <div key={organ.name} className="mb-2">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-3 h-3 rounded-sm ${organ.isInfected ? "bg-[#E24B4A]" : "bg-[#1D9E75]"}`}
                      />
                      <div className="text-[#5DCAA5] text-xs">
                        {organ.name.toUpperCase()}
                      </div>
                    </div>
                    <div className="text-[10px] text-[#3d6b55]">
                      Def: {organ.activeDefenseCount}
                    </div>
                  </div>
                  {organ.isInfected && (
                    <div className="w-full h-1 bg-[#0a1f12] border border-[#1a3a2a] rounded-sm overflow-hidden">
                      <motion.div
                        animate={{
                          width: `${Math.min(100, (organ.reclamationProgress / RECLAMATION_THRESHOLD) * 100)}%`,
                        }}
                        transition={{ duration: 0.5 }}
            ...
            {actions.map((action, index) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() => {
                    if (isDisabled) {
                      soundManager.play("accessDenied");
                      return;
                    }
                    soundManager.play("button");
                    deployDefenseUnit(selectedZone, action.id);
                  }}
                  whileHover={
                    !isDisabled
                      ? { scale: 1.02, boxShadow: `0 0 20px ${action.color}40` }
                      : {}
                  }
                  ...
                  style={{
                    ...
                    backgroundColor: !isDisabled ? "#0a1f12" : "#0a1210",
                  }}
                >
                  <div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isCooldown && (
                        <div className="text-xs font-bold text-[#E24B4A]">
                          CD: {cooldown}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-3">
                    <div className="text-[#5DCAA5] text-xs mb-2 h-8">
                      {action.description}
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="text-[#7F77DD] text-xs">COST:</div>
                      <div
                        className={`text-sm font-bold ${canAfford ? "text-[#7F77DD]" : "text-[#E24B4A]"}`}
                      >
                        {actualCost} EP
                        {timesUsed > 0 && (
      ...
      {showRoundBanner && (
        ...
        >
          ...
          >
            <div className="text-[#1D9E75] text-xl tracking-[12px] mb-2 font-mono">
              INITIATING
            </div>
      ...
      {showStartPopup && !showRoundBanner && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="fixed inset-0 bg-[#050d0a] bg-opacity-95 flex items-center justify-center z-[70]"
        >
          <div className="bg-[#0a1f12] border-2 border-[#E24B4A] rounded-sm p-8 w-[500px] shadow-[0_0_30px_rgba(226,75,74,0.2)]">
            ...
            <div className="text-[#e8f5f0] font-mono text-sm space-y-4 mb-8 text-center border-y border-[#1a3a2a] py-4">
              ...
              <p className="text-[#EF9F27] font-bold">
                Awaiting immune system deployment...
              </p>
            </div>
            <button
              onClick={() => {
          ...
          <div className="bg-[#0a1f12] border-2 border-[#EF9F27] rounded-sm p-10 w-[500px] shadow-[0_0_30px_rgba(239,159,39,0.2)] flex flex-col items-center">
            ...
            {mutationPopup.counter && (
              <div className="w-full bg-[#0d1a0f] border border-[#7F77DD] rounded-sm px-4 py-3 mb-8 text-center">
                <div className="text-[#7F77DD] text-[10px] tracking-[2px] mb-1">
                  RECOMMENDED COUNTER
                </div>
          ...
          <div className="w-full h-full max-w-5xl border-2 border-[#1D9E75] bg-[#0a1f12] rounded-sm p-6 flex flex-col font-mono text-sm shadow-[0_0_30px_rgba(29,158,117,0.2)]">
            <div className="flex items-center justify-between text-[#5DCAA5] tracking-[2px] mb-4 pb-4 border-b border-[#1a3a2a]">
              ...
              <button
                ...
                className="text-[#E24B4A] hover:text-[#ff5c5c] text-lg font-bold tracking-[2px] transition-colors"
              >
                [CLOSE]
              </button>
            </div>
